      ...
      <div className="grid gap-3 md:grid-cols-3">
        {data.variants.map((variant) => (
          <button
            ...
            className="group rounded-xl border border-slate-800 bg-slate-900/80 p-4 text-left transition hover:border-brand-400/40"
          >
            <p className="text-sm font-semibold text-slate-100">{variant.name}</p>
            <div className="mt-3 space-y-2 text-xs text-slate-300">
              <p>CTR: <span className="text-brand-200">{variant.ctr}%</span></p>
              <p>CVR: <span className="text-brand-200">{variant.cvr}%</span></p>
              <p>Sentiment: <span className="text-amber-200">{variant.sentiment}</span></p>
            </div>
...
export function ChannelSelector({ data, onAction }) {
  return (
    <div className="card animate-slide-up">
      <p className="mb-4 flex items-center gap-2 text-sm font-semibold text-slate-200">
