    ...
    <div className="card animate-slide-up">
      ...
      {selected && (
        <div className="mt-5 rounded-xl border border-brand-400/20 bg-[#0f1929] p-4 space-y-3 animate-slide-up">
          ...
          {/* ── Flyer preview: portrait card with full HTML typography overlay ── */}
          {(selected.flyerImageUrl || content.flyer_image_url) && (
            <div
              className="relative group overflow-hidden rounded-2xl border border-slate-700/60 shadow-2xl bg-slate-950"
              style={{ aspectRatio: "2/3", maxHeight: 580 }}
            >
              {/* Background image — decorative only, ALL text rendered below in HTML */}
              <img
                src={selected.flyerImageUrl || content.flyer_image_url}
                alt="Flyer background"
                className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.03]"
              />

              {/* Gradient: transparent top → solid dark at bottom for text legibility */}
              <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-black/20 to-slate-950" />

              {/* Platform pill — top left */}
              {selected.platform && (
                <div className="absolute top-4 left-4">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white/80 bg-black/50 backdrop-blur-md px-2.5 py-1 rounded-full border border-white/10">
                    {selected.platform}
                  </span>
                </div>
              )}
              ...
              {(() => {
                const imgUrl = selected.flyerImageUrl || content.flyer_image_url || "";
                // Cloudinary download URL: insert fl_attachment in the /upload/ path segment.
                // Falls back to a plain anchor download for non-Cloudinary URLs.
                const downloadUrl = imgUrl.includes("/upload/")
                  ? imgUrl.replace("/upload/", "/upload/fl_attachment/")
                  : imgUrl;
                return (
                  <div className="absolute right-3 top-3 flex items-center gap-2 opacity-0 transition-opacity group-hover:opacity-100 z-10">
                    ...
                    {/* Download — forces save dialog via Cloudinary fl_attachment or download attr */}
                    <a
                      href={downloadUrl}
                      download="flyer.jpg"
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1.5 rounded-lg bg-teal-500/80 backdrop-blur-sm px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-teal-500 transition-colors"
                    >
              ...
              {/* ── Typography layout — bottom two-thirds ── */}
              <div className="absolute bottom-0 left-0 right-0 flex flex-col justify-end p-6 space-y-3 pb-7">
                {/* Headline — large, bold, high contrast */}
                {content.headline && (
                  <h2
                    className="text-white leading-tight font-black tracking-tight"
                    style={{
                      fontSize: "clamp(1.4rem, 4.5vw, 2rem)",
                      textShadow: "0 2px 20px rgba(0,0,0,1), 0 1px 4px rgba(0,0,0,1)",
                ...
                {content.cta && (
                  <div>
                    <span
                      className="inline-block bg-teal-400 text-slate-900 font-extrabold uppercase tracking-wider px-5 py-2.5 rounded-lg shadow-2xl"
