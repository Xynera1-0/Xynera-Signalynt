...
async def spy_scout_node(state: ResearchState, db=None) -> dict:
    ...
    if "exa_search" in tools:
        results = await tools["exa_search"](f"{focus} competitor marketing strategy", num_results=5)
        tool_results.extend(results)

    if "tavily_search" in tools:
        results = await tools["tavily_search"](f"{focus} competitor ad campaign 2025 2026", max_results=5)
        tool_results.extend(results)

    if "semrush_keyword_overview" in tools:
        keywords = focus.split()[:2]
        for kw in keywords:
            results = await tools["semrush_keyword_overview"](kw)
            tool_results.extend(results)
    ...
    synthesis_prompt = (
        f"Based on the following competitive intelligence data, produce your findings as JSON "
        f"(same schema as briefed).\n\nFOCUS: {focus}\n\nDATA:\n{tool_context[:8000]}"
    )
